...
class TradingBot(Thread):
    def __init__(self, symbol, interval, ema_slow, ema_fast, adx_period, adx_threshold, chop_period, chop_threshold, initial_trade_percentage, api_key, api_secret):
        ...
        # Convert ADX parameters to integers or set to None if invalid
        try:
            self.adx_period = int(adx_period) if adx_period and int(adx_period) >= 1 else None
            self.adx_threshold = int(adx_threshold) if adx_threshold and int(adx_threshold) >= 1 else None
        except ValueError:
            self.adx_period = None
            self.adx_threshold = None

        # Convert Choppiness parameters to integers or set to None if invalid
        try:
            self.chop_period = int(chop_period) if chop_period and int(chop_period) >= 1 else None
            self.chop_threshold = int(chop_threshold) if chop_threshold and int(chop_threshold) >= 1 else None
        except ValueError:
            self.chop_period = None
            self.chop_threshold = None
    ...
    def on_message(self, ws, message):
        ...
        if is_candle_closed:
            ...
            max_period = max(self.ema_slow, self.ema_fast, self.adx_period if self.adx_period else 0 * 2, self.chop_period if self.chop_period else 0)
    ...
    def send_update(self, message=None):
        ...
        update_data = {
            ...
            "adx_threshold": self.adx_threshold if self.adx_threshold else "Not used",
            "chop_threshold": self.chop_threshold if self.chop_threshold else "Not used"
        }
    ...
    def calculate_required_data_length(self):
        periods = [self.ema_slow, self.ema_fast]
        if self.adx_period:
            periods.append(self.adx_period * 2)
        if self.chop_period:
            periods.append(self.chop_period)
        max_period = max(periods)
        additional_data = max([self.adx_period * 2 if self.adx_period else 0,
                               self.chop_period if self.chop_period else 0])
    ...
    def calculate_indicators(self):
        ...
        # Calculate ADX only if both period and threshold are valid
        if self.adx_period:
            self.calculate_adx(self.adx_period)
        elif 'ADX' in self.df.columns:
            self.df = self.df.drop('ADX', axis=1)

        # Calculate Choppiness only if both period and threshold are valid
        if self.chop_period:
            self.calculate_choppiness(self.chop_period)
        elif 'Choppiness' in self.df.columns:
            self.df = self.df.drop('Choppiness', axis=1)
